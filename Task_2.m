...
for k = 1:length(K_range)
    ...
    for i = 1:length(p_range) % i = placeholder to run for the entire p_range
        ...
        calculatedResults(i) = (K) / ((1 - p)^2); % Formula for calculated results 
        simulatedResults(i) = runTwoSeriesLinkSim(K, p, N); %Call runSingleLink to gather sim data
    end
    ...
    allCalculatedResults{k} = calculatedResults;
...
for k_index = 1:length(K_range)
    K = K_range(k_index);
    simulatedResults = allSimulatedResults{k_index}; %set = to previous array
